"""
Runner for the plain Python microgpt (1. microgpt/main.py) with timing + JSON output.
Runs a configurable number of steps for benchmarking.
"""
...
docs = [l.strip() for l in open(data_path).read().strip().split('\n') if l.strip()]
random.shuffle(docs)
...
with open(os.path.join(os.path.dirname(__file__), 'results_plain.json'), 'w') as f:
    json.dump(results, f)
